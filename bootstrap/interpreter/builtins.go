...
func registerBuiltins(e *Env) {
	e.Define("zombie.internal.builtins", map[string]any{
		...
		"error": builtinProcedure(func(p *process) {
			if p.argc != 1 {
				p.fail(ErrWrongArgCount)
				return
			}
			msg := p.args[0]
			p.fail(errors.New(msg.(string)))
		}),
		...
		"string-empty?": liftProcedure(func(s string) bool {
			return len(s) == 0
		}),
		"string-first": liftProcedure(func(s string) int {
			r, _ := utf8.DecodeRuneInString(s)
			return int(r)
		}),
		"string-rest": liftProcedure(func(s string) string {
			_, n := utf8.DecodeLastRuneInString(s)
			return s[n:]
		}),
		"string-concat": liftProcedure(func(a, b string) string {
			return a + b
		}),
